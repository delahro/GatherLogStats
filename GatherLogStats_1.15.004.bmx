...
'	18/02/2026	1.15.000		Add code to extract R2W folder usage
'								Update Log paramater to be more dynamic and not use just the server name.
'								Add SETCURRENTDATE parameter to override current date.	
'	23/02/2026	1.15.001
'	17/03/2026	1.15.002		Bug fix - t_database.CleanupDB() not being called.	
'	03/06/2026	1.15.003		Add DASHBOARDTIMERANGE	
'	24/06/2026	1.15.004		Amend file names for running counts
'									change prodcounts_Running_ALL.csv to Running_ALL - filecounts.csv
'									change errorcounts_Running_ALL.csv to Running_ALL-errorcounts.csv
'									change filecounts_Running_ALL.csv to Running_ALL-pubcounts.csv			
'
'
' -----------------------------------------------------------------------------------------------------------------
SuperStrict
...
Const version:String = "1.15.004"
Const builddate:String = "24/06/2026"
...
Global dashboardtimerange:Int = 2
Global SETCURRENTDATE:String = Null
Global logpath_current:String = "\c$\Program Files (x86)\Report2Web Client Tools\Logs\"'"\d$\ControlD_RouterLog_temp\"
Global LogFilePath:String = "\\{server}\c$\Program Files (x86)\Report2Web Client Tools\Logs\"
'Global LogFilePath:String = "D:\Projects\BlitzMax\Work\Report2Web\GatherLogStats\logs\{server}\"
Global ServerList:String[] '= ["CFR2WEBPUB02","CFR2WEBPUB03","CFR2WEBPUB04","CFR2WEBPUB05"]

Global StatsFolderList_path:String[] '= ["/Invoices/IGA/NSW/Customer", "/Invoices/IGA/VIC/Customer", "/Invoices/IGA/QLD/Customer", "/Finance/Supplier Portal/Remittances"]
Global StatsFolderList_TimeStamp:String[]
Global StatsFolderList_GroupName:String[]
...
Function Update_Stats()
	...
	Local current_date:String
	If SETCURRENTDATE = Null
		current_date = GetCurrentDate("DDMMYYYY")
	Else
		current_date = SETCURRENTDATE '"15022026" 'GetCurrentDate("DDMMYYYY")
	End If
	...
	Local status:Int
	Local folderpathfound:String
	Local totalDBUpdates:Int = 0
	Local date:String
	For Local i:Int = -days2load To 0 
		...
		logdate = ReformatDate(searchdate, "DDMMYYYY", "YYYY-MM-DD")
		...
		For Local server:String = EachIn ServerList
			...
			log_source_path = Replace(LogFilePath, "{server}", server) + log_filename
			'Print log_source_path
			message(1, "Processing " + log_source_path)
			If FileType(log_source_path) <> 1
				message(1, "*** Error: Log file not found: " + log_source_path)
			Else
				...
				Else
					...
					Local DocFolderPath:String
					Local linedata:String = Null
					Local writeDatestamp:String = Null
					Local entries:Int = Null
					If seekSize > 0
					...
					Repeat 
						...
						If strFind(linedata,"Wrote out file") >0 
							...
							writeDatestamp = reformatDate(logdate, "YYYY-MM-DD", "YYYYMMDD")
							...
							Else
								totalDBUpdates:+1
							End If
							DocFolderPath = ExtractDir(getstring(linedata, "Wrote out file: ", " with"))
							'Print DocFolderPath
							If StatsFolderFound(DocFolderPath, folderpathfound)
								StatsFolder_UpdateStats(folderpathfound, writeDatestamp + endtime)
							End If
						...
						If strFind(linedata,"Published on server")>0 Or strFind(linedata,"File in Error")>0  
							If strFind(linedata,"Published on server") >0
								status = 1
								entries = Int((db_count(db_pntr, "published where doc_name = '" + docname + "' and status='0'")))
...
Function Load_settings()
	...
	Else	
		While Not Eof(cfgfile)
			...
			If StrFound(linedata, "SETCURRENTDATE=")
				SETCURRENTDATE = GetString(linedata, "SETCURRENTDATE=", ";")
				If Not CheckValidDate(SETCURRENTDATE, "DDMMYYYY")
					t_logfile.writelog("Invalid date supplied for SETCURRENTDATE, reset to NULL")
					SETCURRENTDATE = Null
				End If
			End If
			If strFound(linedata, "LOGFILEPATH=")
				LogFilePath = getString(linedata, "LOGFILEPATH=", ";")
			End If
			...
			End If
			If strFind(linedata, "DASHBOARDTIMERANGE=") = 1
				dashboardtimerange = Int(getstring(linedata, "DASHBOARDTIMERANGE=", ";"))
				If dashboardtimerange <= 0 Then dashboardtimerange = 0
			End If
			If strFind(linedata, "recsPerTransaction=")
				...
				End If
			End If
			If strFind(linedata, "FOLDERSTATS=")
				'DebugLog(linedata)
				Local values:String = GetString(linedata, "FOLDERSTATS=", ";")
				Local vals:String[] = values.Split("|")
				Local temppath:String = Replace(vals[0], "\", "/")
				Local tempgroupname:String = vals[1]
				'DebugLog(temppath)
				Local count:Int = Len(StatsFolderList_path)
				StatsFolderList_path = StatsFolderList_path[..count + 1]
				StatsFolderList_TimeStamp = StatsFolderList_TimeStamp[..count + 1]
				StatsFolderList_GroupName = StatsFolderList_GroupName[..count + 1]
				
				StatsFolderList_path[count] = temppath
				StatsFolderList_TimeStamp[count] = "00000000000000"
				StatsFolderList_GroupName[count] = tempgroupname
				DebugLog(count + " " + StatsFolderList_path[count] + " " + StatsFolderList_GroupName[count])
			End If
...
Function Gen_Reports()
	...
	Local col2:String
	'Local col3:String
	Const ralign:String = " align=" + Chr(34) + "right" + Chr(34)
	Const lalign:String = " align=" + Chr(34) + "left" + Chr(34)
...
Function Gen_data_files()
	...
	outpath = work_path + "\Running_ALL-filecounts.csv"
	...
	outpath = work_path + "\Running_ALL-errorcounts.csv"
	DebugLog("outpath=" + outpath)
	t_output.ClearOutput() 'Clear output buffer
	stephour = hour-range
	If stephour < 0 Then stephour = 0 ' Set hour to midnight if value if below zero
	...
	'********************************************************************************************************
	message(1, "  Publication errors all servers")
	outpath = work_path + "\Running_ALL-pubcounts.csv"
	DebugLog("outpath=" + outpath)
	t_output.ClearOutput() 'Clear output buffer
	stephour = hour-range
	If stephour < 0 Then stephour = 0 ' Set hour to midnight if value if below zero
	...
	'********************************************************************************************************
	range = dashboardtimerange	' set to last 2 hours
...
'	t_output.WriteOutput(outpath)
	...
	'********************************************************************************************************
	'*        Generate reporting in R2W Folder action xxxx
	'********************************************************************************************************
	message(1, "  R2W Folder Activity")
	outpath = work_path + "\r2wfolderstats.csv"
	DebugLog("outpath=" + outpath)
	t_output.ClearOutput() 'Clear output buffer
	t_output.Add2Output("groupname,path,datestamp,fileage,ageflag")
	rowTotal = 0
	'command = "select endtime,sum(count) as count from published_counts_by_server where endtime >= '" + Left(searchdate, 8) + "' group by endtime;"
	command = "select folder_path,last_update,groupname from folder_stats order by groupname desc;";
	'Print command
	rows = t_database.QueryDB(command, rc)
	If rc = False
		t_logfile.writeLog(db_last_command)
	...
	Else
		For Local rowdata:String = EachIn rows
			'Print rowdata
			vals = rowdata.Split("|")
			Local folderpath:String = Replace(vals[0], "/", ">")
			Local timestamp:String = ReformatDate(vals[1], "YYYYMMDDhhmmss", "YYYY-MM-DD hh:mm")
			Local groupname:String = Propercase(vals[2])
			Local folderage:Int = GetAgeMinutes(vals[1])
			If folderage < 0
				folderage = 0
			End If
			outtext = groupname + "," + folderpath + "," + timestamp + "," + folderage + ",0"
			t_output.Add2Output(outtext)
		Next
...
Function StatsFolderFound:Int(folderpath:String, folderpathfound:String Var)
	Local result:Int = False
	folderpathfound = Null
	For Local path:String = EachIn StatsFolderList_path
		'Print folderpath + " " + path
		If strfound(folderpath, path)
			result = True
			folderpathfound = path
			Exit
		End If
	Next
	Return result
End Function
Function StatsFolder_UpdateStats:Int(folderpath:String, timestamp:String)
	DebugLog("------------------------------------------------------------------")
	DebugLog(folderpath + " " + timestamp)
	For Local i:Int = 0 To Len(StatsFolderList_path) - 1
		If StatsFolderList_path[i] = folderpath
			If timestamp > StatsFolderList_TimeStamp[i]
				StatsFolderList_TimeStamp[i] = timestamp
			End If
			Exit
		End If
	Next
End Function
Function StatsFolder_UpdateStatsDB:Int()
	For Local i:Int = 0 To Len(StatsFolderList_path) - 1
		DebugLog(LSet(StatsFolderList_path[i], 30) + StatsFolderList_TimeStamp[i])
		Local last_update:String = StatsFolder_getLastUpdate(StatsFolderList_path[i])
		If last_update = Null
			StatsFolder_AddFolder(StatsFolderList_path[i], StatsFolderList_TimeStamp[i], StatsFolderList_GroupName[i])
		Else
			If StatsFolderList_TimeStamp[i] > last_update
				StatsFolder_UpdateFolder(StatsFolderList_path[i], StatsFolderList_TimeStamp[i], StatsFolderList_GroupName[i])
			End If
		End If
	Next
End Function
Function StatsFolder_getLastUpdate:String(folderpath:String)
	Local rc:Int, rows:String[]
	Local command:String
	command = "select last_update from folder_stats where folder_path = '" + folderpath + "';"
	DebugLog(command)
	rows = t_database.QueryDB(command, rc)
	If Len(rows) = 0
		Return Null
	Else
		Return(getString(rows[0], "", "|"))
	End If
End Function
Function StatsFolder_folderExists:Int(folderpath:String)
	Local rc:Int, rows:String[]
	Local command:String
	command = "select count(*) from folder_stats where folder_path = '" + folderpath + "';"
	DebugLog(command)
	rows = t_database.QueryDB(command, rc)
	Return(Int(getString(rows[0], "", "|")))
End Function
Function StatsFolder_AddFolder:Int(folderpath:String, timestamp:String, groupname:String)
	Local rc:Int
	Local command:String
	command = "INSERT INTO folder_stats(folder_path, last_update, groupname) VALUES ('" + folderpath + "', '" + timestamp + "', '" + groupname + "');"
	DebugLog(command)
	t_database.QueryDB(command, rc)
	'Return(Int(getString(rows[0], "", "|")))
End Function
Function StatsFolder_UpdateFolder:Int(folderpath:String, timestamp:String, groupname:String)
	Local rc:Int
	Local command:String
	command = "UPDATE folder_stats SET last_update = '" + timestamp + "', groupname = '" + groupname + "' WHERE folder_path = '" + folderpath + "';"
	DebugLog(command)
	t_database.QueryDB(command, rc)
	'Return(Int(getString(rows[0], "", "|")))
End Function
...
Type t_database
	Function BuildDB()
		...
		Local command:String
		db_command(db_pntr, "PRAGMA journal_mode = WAL;")
		db_command(db_pntr, "PRAGMA cache_size = -64000;")
		...
		db_createTable(db_pntr, "folder_stats", "folder_path TEXT,last_update TEXT,groupname TEXT")
		'End
		'********************************************************************************
		' Views
		db_command(db_pntr, "CREATE VIEW serverlist_view AS select distinct(server) from published order by server;")
		command = "create view duration_view as " + ..
				" select " + ..
				" 	server as server, "+..
